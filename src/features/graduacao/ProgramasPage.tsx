...
// Item vale para a idade filtrada quando é universal (sem faixa) ou é daquela
...
// Reordena `full` trocando o item `id` com o vizinho VISÍVEL na direção dada.
...
// Seletor compacto de faixa etária de um item (só aparece se a cor tem faixas).
...
// Chip clicável de faixa (cor), com a cor real.
function FaixaChip({
  base,
  ativo,
  temPrograma,
  ...
  return (
    <button
      ...
      title={temPrograma ? "Tem programa" : "Sem programa ainda"}
    >
      <span
      ...
      />
      {info.nome}
      {!temPrograma && <span className="text-xs text-muted-foreground">•</span>}
    </button>
  );
}
...
export function ProgramasPage() {
  ...
  // Carrega o programa da faixa selecionada a partir da config (descarta edições
  // não salvas ao trocar de faixa).
  useEffect(() => {
  ...
  function criarPrograma() {
    ...
    toast.info(`Programa da faixa ${info.nome} criado. Edite e salve.`);
  }
  ...
  function removerFaixaEtaria(id: string) {
    if (!prog) return;
    // Solta os itens que apontavam para esta faixa (viram universais).
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2 rounded-md border border-border bg-secondary/40 px-3 py-2 text-sm text-muted-foreground">
        ...
        <p>
          Monte o programa de cada faixa: as faixas etárias, os graus e os
          requisitos e critérios (que podem variar por idade) que o aluno
          demonstra antes de graduar. O conjunto gera a apostila em PDF.{" "}
        ...
        <CardContent className="flex flex-wrap gap-2 p-4">
          {OPCOES_FAIXA_BASE.map((f) => (
            <FaixaChip
              key={f.valor}
              base={f.valor}
              ativo={faixaBase === f.valor}
              temPrograma={cfg?.programas.some((p) => p.faixaBase === f.valor) ?? false}
              onClick={() => setFaixaBase(f.valor)}
            />
          ))}
        </CardContent>
      </Card>
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
            <p className="text-sm text-muted-foreground">
              A faixa {faixaInfo(faixaBase).nome} ainda não tem programa.
            </p>
